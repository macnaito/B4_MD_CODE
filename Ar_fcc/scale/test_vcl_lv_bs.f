...
!　速度こうしんはvで行う　vcl langvine+brendsen
      program ljmd
      ...
      real*8 Treg,gamma_lv,sgm_lv,gaus(3),random(3)
      real*8 Preg,beta,micro
...
!　ランジュバン法　変数
      Treg=50.d0
      gamma_lv=5.d-4
      c=exp(-gamma_lv*0.5d0*dt)
      sgm_lv=sqrt(kb*Treg/amass*(1.d0-c**2))
!　ベレンゼン法　変数
      beta=5.d-2*dt
      Preg=1.d-4/Gp    
...
!　圧力制御
      x=matmul(h,s)
      micro=(1.d0+dt/beta*(gpa(istep)-Pext))**(1.d0/3.d0)
      h=h*micro
      x=x*micro
      call vol_inverse(h,vol,h_inver)
      s=matmul(h_inver,x)
...
!
  ...
  !    write(*,*)istep,tempk,gpa(istep),smap(istep),f+ekin
      write(*,*)istep,tempk,gpa(istep)*Gp,h(1,1)
...
!
! maxwell分布
      subroutine gauss(gaus)
      implicit none
      real*8 gaus(3),u1,u2,pi
      integer i
      ...
      do i=1,3
        ...
        gaus(i)=sqrt(-2.d0*log(u1))*cos(2.d0*pi*u2)
      enddo
      return
      end
!
!　-1~1の乱数    分散１にするため×√３
      subroutine rand(random)
      implicit none
      real*8 u1,random(3)
      integer i
