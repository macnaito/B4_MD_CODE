...
!ファイルの読み込み
     ...
     &   +h_inver(3,3)*x(3,i) 
      ...
      enddo 
      do i=1,n
        x(1,i)=h(1,1)*s(1,i)+h(1,2)*s(2,i)+h(1,3)*s(3,i)
        x(2,i)=h(2,1)*s(1,i)+h(2,2)*s(2,i)+h(2,3)*s(3,i)
        x(3,i)=h(3,1)*s(1,i)+h(3,2)*s(2,i)+h(3,3)*s(3,i)
      enddo
...
!計算start      
     ...
     &        +matmul(gh,s(:,i))
      enddo
      do i=1,n
        ekin=ekin+0.5d0*amass*v(1,i)**2
        ekin=ekin+0.5d0*amass*v(2,i)**2
        ekin=ekin+0.5d0*amass*v(3,i)**2
      enddo
     ...
     &    27.2116*11605d0
        ...
        do j=1,n
          as(:,j)=matmul(h_inver,dfdx(:,j))/amass
          as(:,j)=as(:,j)
     &        -2.d0*matmul(h_inver,matmul(gh,vs(:,j)))
        enddo
